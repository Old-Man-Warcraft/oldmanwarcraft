...
void WorldSessionMgr::UpdateSessions(uint32 const diff)
{
    ...
    for (SessionMap::iterator itr = _sessions.begin(), next; itr != _sessions.end(); itr = next)
    {
        ...
        // In-world sessions are also updated by Map::UpdateOwnedSessions on the map worker thread.
        // Keep visiting them here so the world thread can drain leading thread-unsafe packets and
        // run session-only maintenance (logout, warden, closed sockets) without stealing map-safe
        // packets from the map worker and reordering the queue.
