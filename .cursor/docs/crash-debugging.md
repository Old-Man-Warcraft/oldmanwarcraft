...
./.cursor/scripts/windsurf/analyze-crash.sh
```

### Analyze Specific Crash by Timestamp
```bash
./.cursor/scripts/windsurf/analyze-crash.sh "2026-02-10 14-55-58"
```

### Enable Core Dumps (Better Stack Traces)
```bash
./.cursor/scripts/windsurf/enable-coredumps.sh
```

### Monitor for Crashes in Real-Time
```bash
./.cursor/scripts/windsurf/monitor-crashes.sh
```
...
**Warning**: AddressSanitizer adds ~2x memory overhead and a large CPU cost. **Do not use on live production under player load.** Use a dedicated debug build on a **separate machine**, container, or maintenance window with no players.
...
6. **Confirm the fix** in a safe environment (staging, local repro, or controlled production restart with rollback) before calling the incident closed
