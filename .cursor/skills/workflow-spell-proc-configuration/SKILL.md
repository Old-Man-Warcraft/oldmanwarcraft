---
name: workflow-spell-proc-configuration
description: Workflow for configuring spell_proc entries, validating flags, and testing proc behavior. Use when adding or fixing spell procs.
---
...
- Live or staging server for combat testing (coordinate **production** impact)
- MySQL client for database queries
- Combat log enabled for testing

## MCP (optional)

With the **azerothcore** MCP connected, use `diagnose_spell_proc`, `explain_proc_flags`, `compare_spell_dbc_vs_proc`, `search_spell_procs`, `search_spells_by_proc_flags` before hand-writing SQL. Add **fetch** / **firecrawl** / **exa** for external proc discussions. See `.cursor/reference/mcp-tools-inventory.md`.
...
**View combat log**:
```bash
tail -f env/dist/logs/Server.log | grep -i "proc\|aura"
```
...
grep -i "proc" env/dist/logs/Server.log
