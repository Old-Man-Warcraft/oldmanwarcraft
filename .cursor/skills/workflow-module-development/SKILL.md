---
name: workflow-module-development
description: Workflow for creating a new AzerothCore module (structure, CMake, SQL, testing). Use when scaffolding or extending modules.
---
...
- Plan for **production-safe** validation (backup, rollback, maintenance window) or a **separate staging host** if you use one

## MCP (use when connected)

- **GitLab-MCP**: `create_branch`, `create_merge_request`, `create_or_update_file`, `push_files` (as allowed) for the **module** project on GitLab; `get_merge_request_diffs` for review.
- **GitHub** (if configured): mirror checks against upstream module template or AzerothCore skeleton.
- **azerothcore**: `read_source_file`, `search_azerothcore_source` for `ScriptMgr` / loader patterns; `query_database` for module tables during development.
- **Notion**: document new modules or config rollout steps.
- **sequential-thinking**: scaffold large multi-file module plans.
...
- Validate with backup + rollback before applying on production, or test SQL on staging
...
**Validate on production (or staging)**:
1. Enable module in config; schedule restart if required
2. After start: check `Server.log` / `Errors.log` (or `/data/logs` if mounted) for clean load
3. Exercise critical paths with a GM/test account during a low-impact window
4. Roll back config/binary/SQL if errors appear
...
- Use production-safe validation (backup, staged apply, rollback plan)
...
# Restart production worldserver (coordinate window)
service ac-worldserver restart

# View logs (adjust path if logs live under /data/logs)
tail -f env/dist/logs/Server.log

# Check module loaded
grep -i "module" env/dist/logs/Server.log
