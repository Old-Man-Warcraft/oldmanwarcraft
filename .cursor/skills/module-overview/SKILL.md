...
description: Inventory of project modules, purposes, and key settings. Use when navigating modules, planning features, or explaining the module stack.
---
...
- **GitLab-MCP**: `list_projects`, `get_file_contents`, `list_merge_requests` for **`modules/<name>`** repos and org layout on **origin**.
- **GitHub** (if configured): upstream file/PR search for stock modules vs fork.
- **azerothcore**: `read_source_file`, `search_azerothcore_source` for core integration points; `query_database` only when inspecting schema/data for a module’s SQL.
- **Notion**: internal docs that name modules, owners, or rollout steps.
...
7. **Build and test**: Compile; validate on production with backup/rollback or on separate staging
