-- mod-guild-levels: custom spell for GuildLevels.MountSpeedSpellId (acore_world)
-- SPELL_AURA_MOD_MOUNTED_SPEED_NOT_STACK = 172; ~10% via BasePoints 9 (+1 in effect calc)
SET NAMES utf8mb4;

DELETE FROM `spell_dbc` WHERE `ID` = 80000011;

INSERT INTO `spell_dbc` SELECT
  80000011,
  `Category`,
  `DispelType`,
  `Mechanic`,
  ...
  `SpellDifficultyID`
FROM `spell_dbc` WHERE `ID` = 21353;

UPDATE `spell_dbc` SET
  `Name_Lang_enUS` = 'Guild Perk: Mount Up',
  `Name_Lang_Mask` = 1,
  `EffectAura_1` = 172,
  ...
  `DurationIndex` = 0,
  `SpellIconID` = 136106,
  `AuraDescription_Lang_enUS` = 'Increases mounted movement speed by $s1%.',
