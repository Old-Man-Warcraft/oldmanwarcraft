...
- Treat changes under `src/server/game/` as high-impact by default. Prefer fixes that solve the immediate issue without changing unrelated subsystems or broad shared behavior unless the task clearly requires it.
- Keep generic engine and gameplay logic here; do not move content-specific behavior into scripts just to localize a fix.
- Respect subsystem boundaries already present in the tree. Start by placing changes in the existing owner of the behavior, such as `Entities/`, `Spells/`, `Maps/`, `AI/`, `Handlers/`, or `Server/`.
- When changing spell, map, AI, entity, handler, or session behavior, call out likely regression areas and test systems that directly interact with the modified subsystem or share its dependencies, not only the immediate bug or feature.
...
- Preserve surrounding ownership, lifecycle, and naming patterns. Introduce new abstractions only when the task clearly requires them.
